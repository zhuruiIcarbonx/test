...
require(['jquery', 'jqpagination'], function ($, jqpagination) {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/reportSceneController/page_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
    ...
    //    检验是否已存在相同的编号
    $("#reportScene_number_AMLayer").on('change',function(){
        if(""==$("#reportScene_number_AMLayer").val().trim()){
            $("#detectNum_AMLayer").text("");
            return;
        }
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/reportSceneController/count_ajax.do",
            data: {
                "reportSceneNumber" : $("#reportScene_number_AMLayer").val(),
                "visitKey": window.parent.getVisitKey()
            },
            success: function (msg) {
                if(undefined != msg.visitKeyTimeOut && "true" == msg.visitKeyTimeOut){
...
function search() {
    ...
    $.ajax({
        ...
        data: {
            ...
            "isLastPage": "false",
            "orderBy": "reportSceneNumber asc",
            "reportSceneNumber": $("#search_number").val(),
            "reportSceneName": $("#search_name").val(),
            "reportSceneDescription": $("#search_des").val(),
            "visitKey": window.parent.getVisitKey()
        },
        success: function (msg) {
            if(undefined != msg.visitKeyTimeOut && "true" == msg.visitKeyTimeOut){
...
function add() {
    ...
    $("#ok_btn_AMLayer").on('click', function () {
        if ("" == $("#reportScene_name_AMLayer").val() || "" == $("#reportScene_description_AMLayer").val() || "" == $("#reportScene_number_AMLayer").val()) {
            alert("请填写完整信息后提交！");
            return;
        }
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/reportSceneController/insert_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
                "orderBy": "reportSceneNumber asc",
                "reportSceneNumber": $("#reportScene_number_AMLayer").val(),
                "reportSceneName": $("#reportScene_name_AMLayer").val(),
                "reportSceneDescription": $("#reportScene_description_AMLayer").val(),
                "visitKey": window.parent.getVisitKey()
            },
            success: function (msg) {
                if(undefined != msg.visitKeyTimeOut && "true" == msg.visitKeyTimeOut){
        ...
        $("#reportScene_number_AMLayer").val("");
        $("#reportScene_name_AMLayer").val("");
        $("#reportScene_description_AMLayer").val("");
        $("#detectNum_AMLayer").text("");
    });
...
function modify(data) {
    ...
    $("#reportScene_number_AMLayer").val(dataNumber);
    $("#reportScene_name_AMLayer").val(dataName);
    $("#reportScene_description_AMLayer").val(dataDes);
    $("#reportScene_number_AMLayer").select();
    $("#detectNum_AMLayer").text("√");
    ...
    $("#ok_btn_AMLayer").on('click', function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/reportSceneController/update_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
                "orderBy": "reportSceneNumber asc",
                "reportSceneId": dataID,
                "reportSceneNumber": $("#reportScene_number_AMLayer").val(),
                "reportSceneName": $("#reportScene_name_AMLayer").val(),
                "reportSceneDescription": $("#reportScene_description_AMLayer").val(),
                "visitKey": window.parent.getVisitKey()
...
function del(dataID) {
    ...
    $("#ok_btn_deleteLayer").on('click', function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/reportSceneController/delete_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
...
function showData(msg) {
    ...
    for (var i = 0; i < msg.page.list.length; i++) {
        ...
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder more_content_li'><input type='checkbox' value='" + msg.page.list[i].reportSceneId + "'></li>");
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder large_content_li'>" + msg.page.list[i].reportSceneNumber + "</li>");
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder large_content_li'>" + msg.page.list[i].reportSceneName + "</li>");
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder large_content_li'>" + msg.page.list[i].reportSceneDescription + "</li>");
        $("#ul_" + i).append("<li class='remove_topBorder more_content_li'><a href='javascript:void(0);' onclick=modify('" + msg.page.list[i].reportSceneId + "," + msg.page.list[i].reportSceneNumber + "," + msg.page.list[i].reportSceneName + "," + msg.page.list[i].reportSceneDescription + "')>修改</a><a href='javascript:void(0);' onclick=del('" + msg.page.list[i].reportSceneId + "')>删除</a></li>");

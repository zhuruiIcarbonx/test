...
require(['jquery', 'jqpagination'], function ($, jqpagination) {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/surveyDomainController/page_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
                "orderBy": "surveyDomainName asc",
        ...
        //    检验是否已存在相同的编号
        $("#surveyDomain_number_AMLayer").on('change', function () {
            if(""==$("#surveyDomain_number_AMLayer").val().trim()){
                $("#detectNum_AMLayer").text("");
                return;
            }
            $.ajax({
                type: "POST",
                url: $("#path").val() + "/surveyDomainController/count_ajax.do",
                data: {
                    "surveyDomainNumber": $("#surveyDomain_number_AMLayer").val(),
                    "visitKey": window.parent.getVisitKey()
                },
                success: function (msg) {
                    console.log(msg);
...
function search() {
    ...
    $.ajax({
        ...
        data: {
            ...
            "isLastPage": "false",
            "orderBy": "surveyDomainName asc",
            "surveyDomainNumber": $("#search_number").val(),
            "surveyDomainName": $("#search_name").val(),
            "surveyDomainDescription": $("#search_des").val(),
            "visitKey": window.parent.getVisitKey()
        },
        success: function (msg) {
            if(undefined != msg.visitKeyTimeOut && "true" == msg.visitKeyTimeOut){
...
function add() {
    ...
    $("#ok_btn_AMLayer").on('click', function () {
        if ("" == $("#surveyDomain_name_AMLayer").val() || "" == $("#surveyDomain_description_AMLayer").val()) {
            alert("请填写完整信息后提交！");
            return;
        }
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/surveyDomainController/insert_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
                "orderBy": "surveyDomainName asc",
                "surveyDomainNumber": $("#surveyDomain_number_AMLayer").val(),
                "surveyDomainName": $("#surveyDomain_name_AMLayer").val(),
                "surveyDomainDescription": $("#surveyDomain_description_AMLayer").val(),
                "visitKey": window.parent.getVisitKey()
            },
            success: function (msg) {
                if(undefined != msg.visitKeyTimeOut && "true" == msg.visitKeyTimeOut){
        ...
        $("#surveyDomain_number_AMLayer").val("");
        $("#surveyDomain_name_AMLayer").val("");
        $("#surveyDomain_description_AMLayer").val("");
        $("#detectNum_AMLayer").text("");
    });
...
function modify(data) {
    ...
    $("#surveyDomain_number_AMLayer").val(dataNumber);
    $("#surveyDomain_name_AMLayer").val(dataName);
    $("#surveyDomain_description_AMLayer").val(dataDes);
    $("#surveyDomain_number_AMLayer").select();
    $("#detectNum_AMLayer").text("√");
    ...
    $("#ok_btn_AMLayer").on('click', function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/surveyDomainController/update_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
                "orderBy": "surveyDomainName asc",
                "surveyDomainId": dataID,
                "surveyDomainNumber": $("#surveyDomain_number_AMLayer").val(),
                "surveyDomainName": $("#surveyDomain_name_AMLayer").val(),
                "surveyDomainDescription": $("#surveyDomain_description_AMLayer").val(),
                "visitKey": window.parent.getVisitKey()
...
function del(dataID) {
    ...
    $("#ok_btn_deleteLayer").on('click', function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#path").val() + "/surveyDomainController/delete_ajax.do",
            data: {
                "currentPage": "1",
                "pageSize": "20",
                "isLastPage": "false",
...
function showData(msg) {
    ...
    for (var i = 0; i < msg.page.list.length; i++) {
        ...
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder more_content_li'><input type='checkbox' value='" + msg.page.list[i].surveyDomainId + "'></li>");
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder large_content_li'>" + msg.page.list[i].surveyDomainNumber + "</li>");
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder large_content_li'>" + msg.page.list[i].surveyDomainName + "</li>");
        $("#ul_" + i).append("<li class='remove_topBorder remove_rightBorder large_content_li'>" + msg.page.list[i].surveyDomainDescription + "</li>");
        $("#ul_" + i).append("<li class='remove_topBorder more_content_li'><a href='javascript:void(0);' onclick=modify('" + msg.page.list[i].surveyDomainId + "," + msg.page.list[i].surveyDomainNumber + "," + msg.page.list[i].surveyDomainName + "," + msg.page.list[i].surveyDomainDescription + "')>修改</a>&nbsp;<a href='javascript:void(0);' onclick=del('" + msg.page.list[i].surveyDomainId + "')>删除</a></li>");
